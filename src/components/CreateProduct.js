...
import "../cssClasses/CreateProduct.css"

import tags from '../mock/tags-sample.json'
import {getProduct, updateProduct, useLoading} from "../url/ServerRequest";
...
export const CreateProduct = () => {
    ...
    return <div className='main-form-container'>
            ...
            {({ values }) => (
                <Form className='form-container'>
                    <div className="form-field">
                        ...
                        <Field id="title" name="title" placeholder="Enter the book name" />
                    </div>

                    <div className="form-field">
                        <label htmlFor="description">Descriprion</label>
                        <Field id="description" name="description" placeholder="Enter the description" />
                    </div>
                    ...
                    </div>
                    {
                        Object.entries(values.attributes).map(array =>
                            <div className="form-field">
                                ...
                                <Field id={`attributes.${array[0]}`} name={`attributes.${array[0]}`} placeholder={`Enter ${array[0]}`}/>
                            </div>
                        )
                    }
                    <div className="form-field">
                                        ...
                                        values.sellOptions.map((option, index) => (
                                            <div className="horizontal-container" key={index} >
                                                <div className="form-field">
                                                    ...
                                                    <Field name={`sellOptions.${index}.price`} placeholder="Enter price" type="text" className='horizontal-margin'/>
                                                </div>


                                                <div className="form-field">
                                                    <label htmlFor={`sellOptions.${index}.currency`} className='horizontal-margin'>Currency</label>
                                                    <Field name={`sellOptions.${index}.currency`} placeholder="Enter currency" type="text" className='horizontal-margin'/>
                                                </div>
        ...
        <Button style={{width: "15%", marginTop: "1%", marginBottom: "1.5%"}} variant="outline-dark" size="med" onClick={() => navigate("/products")}>Back</Button>
    </div>
}
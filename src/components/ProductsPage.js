            ...
            <Row gutter={16}>
                {sample.products.map(product =>
                    <Col key={product.usin} span={8} style={{padding: 20}}>
                        <a href={"/product/" + product.usin}>
                            <Card
                                hoverable
                                style={{ width: 240 }}
                                onClick
                                cover={<img width="100" height="350" alt="example" src={product.images[0]} />}
                            >
                                <Meta title={product.title} description={"by " + product.attributes.author} />
                                <Rate disabled allowHalf defaultValue={sumRating(product.ratings)[1]}/>
                                <span className="ant-rate-text">{"("+sumRating(product.ratings)[0]+")"}</span>
                            </Card>
                        </a>
                    </Col>)}
            </Row>

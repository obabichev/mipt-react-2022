 ...
 */
const getInnerElement = (text, link) => (
...
const TestReactComponent = (props) => {
  ...
  return (
    <div className="red-background" style={{ padding: 50 }}>
      <a href={props.link}>
        {props.children ? props.children : props.link.toUpperCase()}
      </a>
    </div>
  );
};
...
const ConditionalRenderingComponent = (props) => {
  if (props.value && props.value > 10) {
    return (
      <div style={{ backgroundColor: 'red' }}>More than 10 ({props.value})</div>
    );
  }

  return (
    <div>
      No more than 10 ({props.value})
      {typeof props.value !== 'number' && <div>Because it is not number</div>}
    </div>
  );
};
  ...
  render() {
    return (
      <div>
        ...
        {this.props.value > 0 && (
          <TestClassComponent value={this.props.value - 1} />
        )}
      </div>
    );
  }
